							  ...
							  PurpleLogSizeCallback cb, void *data)
{
	...
	if (db_thread_func[PURPLE_DATABASE_LOGGER_WRITE] != NULL) {
		...
		purple_debug_info("Database Logger", "Write function start callback[main thread]\n");
...
static void db_write_notify(gpointer data) 
{
	...
	purple_debug_info("Database Logger", "write notification [from main thread]\n");
	if (op->cb != NULL) {
...
static void db_read_notify(gpointer data) 
{
	...
	purple_debug_info("Database Logger", "read notification [from main thread]\n");
	if (op->cb != NULL) {
...
static int db_get_buddy_id(PurpleLogType type, const char *name, PurpleAccount *account) 
{
	...
	purple_debug_info("Database logger", "get_buddy_id (%s) [new thread]\n", buddy_name);
								...
								time_t datetime, PurpleLog *log, char **output)
{
	...
	purple_debug_info("Database Logger", "append_message_to_output: %s\n", message);
...
static gpointer db_read(gpointer data)
{
	...
	if (!conv_info || conv_info->id == -1)
		...
		purple_debug_info("Database Logger", "---- XXX ---- db_read: process query select\n");
		dres = dbi_conn_queryf(db_logger->db_conn, 
