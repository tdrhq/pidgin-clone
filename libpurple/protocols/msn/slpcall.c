...
msn_slp_process_msg(MsnSlpLink *slplink, MsnSlpMessage *slpmsg)
{
	...
	if (slpmsg->flags == 0x0)
	{
		...
		if (slpmsg->session_id==64) {
			const char *msgid, *start;
			int charsize;
			/*Just to be evil they put a 0 in the string just before the data you want, and then convert to utf-16*/
			body_str = g_utf16_to_utf8(body, body_len/2, NULL, NULL, NULL);
			start = body+(strlen(body_str)+1)*2;
			charsize = (body_len/2)-(strlen(body_str)+1);
			g_free(body_str);
			body_str = g_utf16_to_utf8(start, charsize, NULL, NULL, NULL);
			msgid = g_strdup_printf("{handwritten:%d}", slpmsg->id);
			msn_handwritten_msg_show(slpmsg->slplink->swboard, msgid, 
										body_str+7, slplink->remote_user);
			g_free(msgid); 
		} else {
			body_str = g_strndup((const char *)body, body_len);
