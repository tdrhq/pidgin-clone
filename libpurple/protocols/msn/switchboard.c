...
datacast_msg(MsnCmdProc *cmdproc, MsnMessage *msg)
{
	...
	if (!strcmp(id, "1")) {
		...
		if (swboard->current_users > 1 ||
			((swboard->conv != NULL) &&
			 purple_conversation_get_type(swboard->conv) == PURPLE_CONV_TYPE_CHAT))
			purple_prpl_got_attention_in_chat(purple_account_get_connection(account), swboard->chat_id, user, MSN_NUDGE);

		else
			purple_prpl_got_attention(purple_account_get_connection(account), user, MSN_NUDGE);
